...
def test_verification_status_outside_workspace_is_not_applicable(tmp_path, monkeypatch):
    # agent.coding_context._marker_root/_git_root walk real ancestor
    # directories (capped at depth 6) looking for project markers or a .git
    # dir. This box has real /AGENTS.md and /CLAUDE.md sitting at the
    # filesystem root (both are project markers), so tmp_path — shallow
    # enough under /tmp/pytest-of-.../... to reach '/' within the depth cap
    # — gets falsely detected as inside a workspace, and verification_status
    # proceeds to compute a real status instead of the not_applicable this
    # test expects for a bare, non-workspace tmp dir. Bound both walkers to
    # tmp_path's own tree, matching the isolation fixture in
    # tests/agent/test_coding_context.py for the same underlying cause.
    ...
    monkeypatch.setattr(cc, "_marker_root",
                         lambda cwd: (lambda r: r if _within_tmp(r) else None)(real_marker_root(cwd)))
    monkeypatch.setattr(cc, "_git_root",
                         lambda cwd: (lambda r: r if _within_tmp(r) else None)(real_git_root(cwd)))
